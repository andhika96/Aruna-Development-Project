...
require BASEPATH.'/libraries/Jwt/vendor/firebase/php-jwt/src/JWT.php';
require BASEPATH.'/libraries/Jwt/vendor/firebase/php-jwt/src/BeforeValidException.php';
require BASEPATH.'/libraries/Jwt/vendor/firebase/php-jwt/src/ExpiredException.php';
require BASEPATH.'/libraries/Jwt/vendor/firebase/php-jwt/src/SignatureInvalidException.php';
...
class RestController
{
	...
	*/
	...
	{
		// Load Config Rest
		$this->config = load_cs_config('rest');
		...
		// Load Language Library from System
		$this->lang =& load_class('Lang', 'system');
		...
		$this->_get_args = array_merge($this->_get_args, $GLOBALS['segments']);

		// log_message('debug', json_encode($this->_get_args));
		...
		{
			$this->db = load_db($this->config->item('rest_database_group'), 'MySQL');
		}
	...
	*/
	...
	public function _remap($object_called, $arguments = [])
	{
		...
		// Remove last part of the string with underscore
		// Added by Andhika Adhitia N
		$remove_last_part = substr($object_called, 0, strrpos($object_called, "_"));

		// Remove the first part of the string after slash
		// Added by Andhika Adhitia N
		$remove_last_part2 = substr($remove_last_part, strpos($remove_last_part, '/')+1);

		$controller_method = $remove_last_part2.'_'.$this->request->method;
		...
		if ( ! method_exists($this, $controller_method)) 
		{
			...
			array_unshift($arguments, $get_last_object_called);
		}
		*/
		...
		try 
		{
			...
			// Because the arguments only read the controller or method so you cannot setup the argument in the function
			/*
	...
	*/
	...
	protected function _detect_api_key()
	{
		...
		if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) 
		{
			...
			$res = $this->db->sql_prepare("select * from ".$this->config->item('rest_keys_table')." where ".$this->config->item('rest_key_column')." = :".$this->config->item('rest_key_column')."");
			$bindParam = $this->db->sql_bindParam([$this->config->item('rest_key_column') => $key], $res);
			$row = $this->db->sql_fetch_single($bindParam, 'object');
	...
	*/
	...
	protected function _log_request($authorized = false)
	{
		...
		$is_inserted = $this->db->sql_insert(
			[
	...
	*/
	...
	protected function _check_limit($controller_method)
	{
		...
		$res = $this->db->sql_prepare("select * from ".$this->config->item('rest_limits_table')." where uri = :uri and api_key = :api_key");
		$bindParam = $this->db->sql_bindParam(['uri' => $limited_uri, 'api_key' => $api_key], $res);
		$row = $this->db->sql_fetch_single($bindParam, 'object'); 
		...
		elseif ($row->hour_started < (time() - $time_limit)) 
		{
			// Reset the started period and count
			$this->db->sql_update(['hour_started' => time(), 'count' => 1], $this->config->item('rest_limits_table'), ['uri' => $limited_uri, 'api_key' => $api_key]);
		}
		...
		else 
		{
			...
			$res_update_count = $this->db->sql_prepare("update ".$this->config->item('rest_limits_table')." set count = count+1, hour_started = :hour_started where uri = :uri and api_key = :api_key");
			$bindParam_update_count = $this->db->sql_bindParam(['hour_started' => time(), 'uri' => $limited_uri, 'api_key' => $api_key], $res_update_count);
		}
	...
	*/
	...
	protected function _auth_override_check()
	{
		...
		if ( ! empty($auth_override_class_method)) 
		{
			...
			if ( ! empty($auth_override_class_method[$this->uri->get_segment(1)]['*'])) 
			{ // Check for class overrides

				// No auth override found, prepare nothing but send back a TRUE override flag
				if ($auth_override_class_method[$this->uri->get_segment(1)]['*'] === 'none') 
				{
					return true;
				}

				// Basic auth override found, prepare basic
				if ($auth_override_class_method[$this->uri->get_segment(1)]['*'] === 'basic') 
				{
					$this->_prepare_basic_auth();
				...
				if ($auth_override_class_method[$this->uri->get_segment(1)]['*'] === 'digest') 
				{
					$this->_prepare_digest_auth();
				...
				if ($auth_override_class_method[$this->uri->get_segment(1)]['*'] === 'session')
				{
					$this->_check_php_session();
				...
				if ($auth_override_class_method[$this->uri->get_segment(1)]['*'] === 'jwt')
				{
					$this->_check_jwt();
				...
				if ($auth_override_class_method[$this->uri->get_segment(1)]['*'] === 'whitelist') 
				{
			...
			if ( ! empty($auth_override_class_method[$this->uri->get_segment(1)][$this->uri->get_segment(2)])) 
			{
				// None auth override found, prepare nothing but send back a TRUE override flag
				if ($auth_override_class_method[$this->uri->get_segment(1)][$this->uri->get_segment(2)] === 'none') 
				{
					return true;
				}

				// Basic auth override found, prepare basic
				if ($auth_override_class_method[$this->uri->get_segment(1)][$this->uri->get_segment(2)] === 'basic') 
				{
					$this->_prepare_basic_auth();
				...
				// Digest auth override found, prepare digest
				if ($auth_override_class_method[$this->uri->get_segment(1)][$this->uri->get_segment(2)] === 'digest') 
				{
					$this->_prepare_digest_auth();
				...
				// Session auth override found, check session
				if ($auth_override_class_method[$this->uri->get_segment(1)][$this->uri->get_segment(2)] === 'session') 
				{
					$this->_check_php_session();
				...
				// JWT auth override found, check jwt
				// if ($auth_override_class_method[$this->uri->get_segment(1)][$this->uri->get_segment(2)] === 'jwt')
				if ($auth_override_class_method[$this->uri->get_segment(1)][$this->uri->get_segment(2)] === 'jwt')
				{
					$this->_check_jwt();
				...
				// Whitelist auth override found, check client's ip against config whitelist
				if ($auth_override_class_method[$this->uri->get_segment(1)][$this->uri->get_segment(2)] === 'whitelist') 
				{
					$this->_check_whitelist_auth();
		...
		{
			// check for wildcard flag for rules for classes
			if ( ! empty($auth_override_class_method_http[$this->uri->get_segment(1)]['*'][$this->request->method])) 
			{
				// None auth override found, prepare nothing but send back a TRUE override flag
				if ($auth_override_class_method_http[$this->uri->get_segment(1)]['*'][$this->request->method] === 'none') 
				{
					return true;
				}

				// Basic auth override found, prepare basic
				if ($auth_override_class_method_http[$this->uri->get_segment(1)]['*'][$this->request->method] === 'basic') 
				{
					$this->_prepare_basic_auth();
				...
				// Digest auth override found, prepare digest
				if ($auth_override_class_method_http[$this->uri->get_segment(1)]['*'][$this->request->method] === 'digest') 
				{
					$this->_prepare_digest_auth();
				...
				// Session auth override found, check session
				if ($auth_override_class_method_http[$this->uri->get_segment(1)]['*'][$this->request->method] === 'session') 
				{
					$this->_check_php_session();
				...
				// JWT auth override found, check jwt
				if ($auth_override_class_method_http[$this->uri->get_segment(1)]['*'][$this->request->method] === 'jwt')
				{
					$this->_check_jwt();
				...
				// Whitelist auth override found, check client's ip against config whitelist
				if ($auth_override_class_method_http[$this->uri->get_segment(1)]['*'][$this->request->method] === 'whitelist') 
				{
					$this->_check_whitelist_auth();
			...
			if ( ! empty($auth_override_class_method_http[$this->uri->get_segment(1)][$this->uri->get_segment(2)][$this->request->method])) 
			{
				// None auth override found, prepare nothing but send back a TRUE override flag
				if ($auth_override_class_method_http[$this->uri->get_segment(1)][$this->uri->get_segment(2)][$this->request->method] === 'none') 
				{
					return true;
				}

				// Basic auth override found, prepare basic
				if ($auth_override_class_method_http[$this->uri->get_segment(1)][$this->uri->get_segment(2)][$this->request->method] === 'basic') 
				{
					$this->_prepare_basic_auth();
				...
				// Digest auth override found, prepare digest
				if ($auth_override_class_method_http[$this->uri->get_segment(1)][$this->uri->get_segment(2)][$this->request->method] === 'digest') 
				{
					$this->_prepare_digest_auth();
				...
				// Session auth override found, check session
				if ($auth_override_class_method_http[$this->uri->get_segment(1)][$this->uri->get_segment(2)][$this->request->method] === 'session') 
				{
					$this->_check_php_session();
				...
				// JWT auth override found, check jwt
				if ($auth_override_class_method_http[$this->uri->get_segment(1)][$this->uri->get_segment(2)][$this->request->method] === 'jwt')
				{
					$this->_check_jwt();

					// log_message('debug', $this->_check_jwt());

					return TRUE;
				}

				// Whitelist auth override found, check client's ip against config whitelist
				if ($auth_override_class_method_http[$this->uri->get_segment(1)][$this->uri->get_segment(2)][$this->request->method] === 'whitelist') 
				{
					$this->_check_whitelist_auth();
	...
	*/
	...
	protected function _check_jwt()
	{
		...
		if ( ! empty($authorization))
		{
			...
			{
				$this->response([
				$this->config->item('rest_status_field_name') => self::HTTP_UNPROCESSABLE_ENTITY,
	...
	*/
	...
	protected function _log_access_time()
	{
		...
		return $this->db->update($payload, $this->config->item('rest_logs_table'), ['id' => $this->_insert_id]);
	}
	...
	*/
	...
	protected function _check_access()
	{
		...
		$res = $this->db->sql_prepare("select * from ".$this->config->item('rest_access_table')." where key = :key and controller = :controller");
		$bindParam = $this->db->sql_bindParam(['key' => $this->rest->key, 'controller' => $controller], $res);
		$row = $this->db->sql_fetch_single_array($bindParam);

		if ( ! empty($row) && ! empty($row['all_access'])) 
		{
			return true;
		}
	...
	*/
	...
	protected function _check_cors()
	{
		...
		if ($this->input->method() === 'options') 
		{
			...
			if ( ! isset($this->db) && $this->config->item('rest_enable_logging')) 
			{
				$this->db = load_db($this->config->item('rest_database_group'), 'MySQL');
			}
