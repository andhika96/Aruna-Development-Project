...
class RestController
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->config->item('rest_database_group') && ($this->config->item('rest_enable_keys') || $this->config->item('rest_enable_logging'))) 
        {
            ...
            $this->rest->db = load_db($this->config->item('rest_database_group'), 'MySQL');
        }
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        // And split url string with slash
        // Added by Andhika Adhitia N
        $object_called = preg_replace('/_[A-Za-z0-9].+/s', '', $object_called);
        $object_called = preg_split("#/#", $object_called);

        // Always get last of url string from object_called variable
        $get_last_object_called = end($object_called);

        $controller_method = $get_last_object_called.'_'.$this->request->method;
        
        // log_message('debug', $this->rest->key);
        ...
        {
            // log_message('debug', $controller_method);
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            $res = $this->rest->db->sql_prepare("select * from ".$this->config->item('rest_keys_table')." where ".$this->config->item('rest_key_column')." = :".$this->config->item('rest_key_column')."");
            $bindParam = $this->rest->db->sql_bindParam([$this->config->item('rest_key_column') => $key], $res);
            $row = $this->rest->db->sql_result($bindParam, 'object');

            if ( ! $this->rest->db->sql_counts($bindParam))
            {
                return false;
            }
    ...
    protected function _check_limit($controller_method)
    {
        ...
        // log_message('debug', $this->methods[$controller_method]['limit'].' asd');
        ...
        $res = $db->sql_prepare("select * from ar_limits where uri = :uri and api_key = :api_key");
        $bindParam = $db->sql_bindParam(['uri' => $limited_uri, 'api_key' => $api_key], $res);
        $row = $db->sql_result($bindParam, 'object'); 
        ...
        elseif ($row->hour_started < (time() - $time_limit)) 
        {
            ...
            $this->rest->db->sql_update(['hour_started' => time(), 'count' => 1], $this->config->item('rest_limits_table'), ['uri' => $limited_uri, 'api_key' => $api_key]);
        }
        ...
        else 
        {
            ...
            $res_update_count = $this->rest->db->sql_prepare("update ".$this->config->item('rest_limits_table')." set count = count+1, hour_started = :hour_started where uri = :uri and api_key = :api_key");
            $bindParam_update_count = $this->rest->db->sql_bindParam(['hour_started' => time(), 'uri' => $limited_uri, 'api_key' => $api_key], $res_update_count);
        }
    ...
    protected function _check_whitelist_auth()
    {
        ...
        array_push($whitelist, '8.8.8.8', '8.8.8.0');
