<?php
	 ...
	 *	Source file from CodeIgniter v3.1.13
	 *	Website: https://www.sosiaku.gq
	 *	Website: https://www.aruna-dev.id
	 *	Created and developed by Andhika Adhitia N
	 *	Information file: Session Class
	 */
...
class ARUNA_Session {
	...
	public function __construct(array $params = array())
	{
		...
		$class = $this->_ar_load_classes($this->_driver);
		...
		$wrapper = new ARUNA_SessionWrapper($class);
		...
		{
			if ( ! isset($_SESSION['__ar_last_regenerate']))
			{
				$_SESSION['__ar_last_regenerate'] = time();
			}
			elseif ($_SESSION['__ar_last_regenerate'] < (time() - $regenerate_time))
			{
	 ...
	 * classes. It kind of emulates the AR_Driver library, but is
	 * self-sufficient.
	 *
	...
	protected function _ar_load_classes($driver)
	{
		...
		require_once(BASEPATH.'libraries/Session/ARUNA_Session_driver_interface.php');
		$wrapper = is_php('8.0') ? 'PHP8SessionWrapper' : 'OldSessionWrapper';
		require_once(BASEPATH.'libraries/Session/'.$wrapper.'.php');

		$prefix = config_item('subclass_prefix');

		if ( ! class_exists('ARUNA_Session_driver', FALSE))
		{
			require_once(
		...
		if ( ! class_exists('ARUNA_'.$class, FALSE))
		{
			...
			if ( ! class_exists('ARUNA_'.$class, FALSE) && ! class_exists($class, FALSE))
			{
	...
	protected function _ar_init_vars()
	{
		if ( ! empty($_SESSION['__ar_vars']))
		{
			$current_time = time();

			foreach ($_SESSION['__ar_vars'] as $key => &$value)
			{
				if ($value === 'new')
				{
					$_SESSION['__ar_vars'][$key] = 'old';
				}
				elseif ($value === 'old' || $value < $current_time)
				{
					unset($_SESSION[$key], $_SESSION['__ar_vars'][$key]);
				}
			}

			if (empty($_SESSION['__ar_vars']))
			{
				unset($_SESSION['__ar_vars']);
			}
		}
	...
	public function mark_as_flash($key)
	{
		if (is_array($key))
		{
			...
			$_SESSION['__ar_vars'] = isset($_SESSION['__ar_vars'])
				? array_merge($_SESSION['__ar_vars'], $new)
				: $new;
	...
	{
		if ( ! isset($_SESSION['__ar_vars']))
		{
			return array();
		}

		$keys = array();
		foreach (array_keys($_SESSION['__ar_vars']) as $key)
		{
			is_int($_SESSION['__ar_vars'][$key]) OR $keys[] = $key;
		}
	...
	public function unmark_flash($key)
	{
		...
		foreach ($key as $k)
		{
			if (isset($_SESSION['__ar_vars'][$k]) && ! is_int($_SESSION['__ar_vars'][$k]))
			{
				unset($_SESSION['__ar_vars'][$k]);
			}
		}
	...
	public function mark_as_temp($key, $ttl = 300)
	{
		...
		if (is_array($key))
		{
			...
			$_SESSION['__ar_vars'] = isset($_SESSION['__ar_vars'])
				? array_merge($_SESSION['__ar_vars'], $temp)
				: $temp;
	...
	public function get_temp_keys()
	{
		...
		foreach (array_keys($_SESSION['__ar_vars']) as $key)
		{
			is_int($_SESSION['__ar_vars'][$key]) && $keys[] = $key;
		}
	...
	public function flashdata($key = NULL)
	{
		if (isset($key))
		{
			return (isset($_SESSION['__ar_vars'], $_SESSION['__ar_vars'][$key], $_SESSION[$key]) && ! is_int($_SESSION['__ar_vars'][$key]))
				? $_SESSION[$key]
				: NULL;
		}
