...
require APPPATH.'/vendor/firebase/php-jwt/src/Key.php';
...
class RestController extends \Aruna_Controller
{
    /**
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->config->item('rest_database_group') && ($this->config->item('rest_enable_keys') || $this->config->item('rest_enable_logging'))) {
            $this->rest->db = $this->load->database_codeigniter($this->config->item('rest_database_group'), true);
        }

        // Use whatever database is in use (isset returns FALSE)
        elseif (property_exists($this, 'db_alt')) {
            $this->rest->db = $this->db_alt;
        }
        ...
        if ( ! empty($authorization))
        {
            if (preg_match('/Bearer\s(\S+)/', $authorization, $matches))
            {
                ...
                try
                {
                    ...
                    $decoded = JWT::decode($bearer, new Key($this->jwt_secret_key, 'HS256'));
                }
                    catch (Exception $e)
                {
                    $this->response([
    ...
    */
    ...
    {
        return JWT::encode($token, $this->jwt_secret_key, 'HS256');
    }
    ...
    */
    ...
    protected function jwt_decode($token)
    {
        ...
        $decoded = JWT::decode($token, new Key($this->jwt_secret_key, 'HS256'));
